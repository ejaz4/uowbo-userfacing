...
export const POST = async (req: NextRequest) => {
    ...
    if (botRequest.status == 404) {
        return new NextResponse(JSON.stringify({ error: "Bot request failed", moveTo: "/verify/not-found" }), { status: 500 })
    }

    if (!botRequest.ok) {
        return new NextResponse(JSON.stringify({ error: "Bot request failed" }), { status: 500 })
    }

    const botResponse = await botRequest.json() as { userId: string; displayAvatarURL: string; }
    ...
    if (!userHandover) {
        return new NextResponse(JSON.stringify({ error: "Failed to create handover" }), { status: 500 })
    }
